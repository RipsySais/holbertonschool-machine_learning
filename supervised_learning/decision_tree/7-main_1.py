...
def circle_of_clouds(
        n_clouds, n_objects_by_cloud, radius=1, sigma=None,
        seed=0, angle=0):
    """
    This function returns a dataset made of 'n_clouds' classes.
    Each class is a small gaussian cloud containing 'n_objects_by_cloud'
    points. The centers of the clouds are regularly disposed on a circle
    of radius 'radius' (and center (0,0)). The spreadth of the clouds is
    governed by 'sigma'.
    """
    ...
    def cloud():
        return (rng.normal(size=2 * n_objects_by_cloud) * sigma).reshape(
            n_objects_by_cloud, 2) + np.array([radius, 0])
    ...
    return np.concatenate(
        [np.array(rotate(cloud(), k)) for k in range(n_clouds)],
        axis=0), target()


def iris():
    """ Returns the explanatory features and the target of the famous
    iris dataset """
...
def wine():
    """ Returns the explanatory features and the target of the wine
    dataset """
...
def split(explanatory, target, seed=0, proportion=.1):
    """ Returns a dictionary containing a training dataset and a test
    dataset """
    rng = np.random.default_rng(seed)
...
for d, name in zip(
    ...
    print(f"    - Accuracy on test          :"
          f" {T.accuracy(d['test_explanatory'], d['test_target'])}")
print("-" * 52)
